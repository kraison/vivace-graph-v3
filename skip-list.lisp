         ...
         (let ((pred (%sl-head skip-list)))
           (let ((curr (read-skip-node skip-list (aref (%sn-pointers pred) level))))
             ...
             (when (and (not the-node)
               ...
               (setq the-node curr
                     level-found level)
               (dbg "FOUND ~A/~A AT LEVEL ~A" key value level-found))
             (if (and the-node (not preds) (not succs))
                 (return)
                 (when (and preds succs)
                   (setf (aref preds level) pred
...
(defun add-to-skip-list (skip-list key value)
  (let ((top-level (random-level (%sl-max-level skip-list)))
    ...
    (loop
       ...
       ;;(dbg "~S / ~S:~%  ~S~%  ~S~%" key value (elt preds 0) (elt succs 0))
         ...
         (let* ((head (make-head sl
           ...
           (time
            (dolist (id ids)
              ...
              (incf i)))
           (dbg "~A" (time (find-in-skip-list sl 500)))
           (let ((list (time (skip-list-to-list sl))))
             (dbg "SL SIZE: ~A (should be ~A)" (length list) (%sl-length sl))
             (dbg "~A" (find-in-skip-list sl 0)))
           (dbg "DELETING")
           (time (remove-from-skip-list sl 500))
           (dbg "FOUND: ~A" (find-in-skip-list sl 500))
    ...
    (unwind-protect
         (let ((sl (make-skip-list :heap heap
           ...
           (let* ((ids (loop for i from 1 to 1000 collecting (string-uuid (gen-id))))
                  (i 0))
             (dbg "ABOUT TO START ADDING ITEMS")
             (time
              (dolist (id ids)
                (dbg "ADDING ~S:~S" id i)
                (add-to-skip-list sl (format nil "~12,'0d" i) id)
                (when (memory-magic-byte-corrupt-p heap)
                  (dbg "Heap corrupt: 0x~X" (memory-magic-byte-corrupt-p heap))
                  (error "Corrupt heap!"))
                (incf i)))
             (dbg "~A" (time (find-in-skip-list sl (format nil "~12,'0d" 500))))
             (let ((list (time (skip-list-to-list sl))))
               (dbg "SL SIZE: ~A (should be ~A)" (length list) (%sl-length sl))
               ;;(dbg "~A" (find-in-skip-list sl (format nil "~12,'0d" 0)))
               )
             (terpri)
             (dolist (i '(1 500 100 999))
               (let ((node (find-in-skip-list sl (format nil "~12,'0d" i)))
                 ...
                 (dbg "~S~%" node)
                 (dbg "REPLACING ~S~%   WITH ~S" (%sn-value node) new-id)
                 ;;(dbg "~S" (update-in-skip-list sl (%sn-key node) new-id))
                 (dbg "REMOVE: ~S" (remove-from-skip-list sl (%sn-key node)))
                 ;;(dbg "~{~A~^~%~}~%" (skip-list-to-list sl))
                 (sleep 1)
                 (when (memory-magic-byte-corrupt-p heap)
                   (dbg "Heap corrupt: 0x~X" (memory-magic-byte-corrupt-p heap))
                   (error "Corrupt heap!"))
                 (dbg "FIND: ~S" (find-in-skip-list sl (format nil "~12,'0d" i)))
                 (dbg "~S" sl)
                 (sleep 1)))))
      (progn
             ...
             (time
              (dolist (id ids)
                ...
                (when (memory-magic-byte-corrupt-p heap)
                  ...
                  (error "Corrupt heap!"))
                (incf i)))
             (dbg "~A" (time (find-in-skip-list sl (format nil "~12,'0d" 5))))
             (let ((list (time (skip-list-to-list sl))))
               (dbg "SL SIZE: ~A (should be ~A)" (length list) (%sl-length sl))
               ;;(dbg "~A" (find-in-skip-list sl (format nil "~12,'0d" 0)))
               )
