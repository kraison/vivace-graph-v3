...
(test new-graph-has-spatial-index
  ...
  (with-test-graph (g)
    ...
    (spatial-index-insert (spatial-index g) 1 (make-point 37.1724d0 49.2020d0))
    (is (member 1 (spatial-index-query-bbox (spatial-index g)
  ...
  (with-temp-directory (dir)
    (let ((path (namestring dir)))
      (let ((g (make-graph *integration-graph-name* path :buffer-pool-size 1000)))
        (let ((*graph* g))
          (spatial-index-insert (spatial-index g) 1 (make-point 37.1724d0 49.2020d0))
          (spatial-index-insert (spatial-index g) 2 (make-point 23.7183d0 50.0263d0))
        ...
        (unwind-protect
             (let ((*graph* g2))
               ...
               (let ((cands (spatial-index-query-bbox (spatial-index g2)
                                                      37.16d0 49.19d0 37.19d0 49.21d0)))
                 (is (member 1 cands) "Kharkiv point persisted")
                 (is (not (member 2 cands)) "Lviv point is outside the query window")))
