...
To put a node type on the map, give it a geometry and specialize the ~node-geometry~ generic function to return it. ~node-geometry~ defaults to ~nil~ (a node has no location); when you specialize it for your type, every create/update/delete of such a node automatically maintains the graph's spatial index — there is no separate "add to index" call.
...
#+BEGIN_SRC lisp
  ...
  ;; This one method is all it takes to make observations spatially indexed.
...
~find-near/4~ takes ~?node lat lon radius-metres~; ~find-within/2~ takes ~?node area~.

*** Under the hood, and a few caveats

- *The index* is a skip list (in the graph's heap) keyed by fixed-precision *geohash* strings; each node id is stored under every cell its geometry occupies (duplicates allowed). Because a geohash prefix is a nested bounding cell, window and proximity queries become ordered prefix range scans. Precision is configurable (default 7, ≈150 m cells; 9 ≈ 5 m) when the index is created.
...
- ~node-geometry (node)~ ::
  Generic function (default ~nil~); specialize it for your geometry-bearing node types so they are maintained automatically in the spatial index.
...
- ~(find-near ?node lat lon radius)~ / ~(find-within ?node area)~ ::
