...
~:direction~ is ~:out~ (default), ~:in~, or ~:both~; ~:edge-type~ restricts to one edge type; ~:weight-fn~ overrides how an edge's cost is read. Dijkstra/A* require non-negative weights.
